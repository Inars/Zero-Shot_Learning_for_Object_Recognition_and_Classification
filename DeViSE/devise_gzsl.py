...
class DeViSE():
	...
	def update_W(self, X, labels, sig, W, idx, train_classes):
		...
		for j in idx:
			...
			for i, label in enumerate(y_):
				score = margin+np.dot(XW, sig[:, label])-gt_class_score
				if score>0:
	...
	def zsl_acc_gzsl(self, X, W, y_true, classes, sig, is_seen): # Class Averaged Top-1 Accuarcy
		...
		########################################## UPDATE #################################################
		if is_seen:
			a_file = open("testing/gzsl/devise_dist_seen_"+self.args.dataset+".txt", "w")
			b_file = open("testing/gzsl/devise_pred_seen_"+self.args.dataset+".txt", "w")
			np.savetxt(a_file, class_scores)
			np.savetxt(b_file, y_pred)
			a_file.close()
			b_file.close()
		if not is_seen:
			a_file = open("testing/gzsl/devise_dist_unseen_"+self.args.dataset+".txt", "w")
			b_file = open("testing/gzsl/devise_pred_unseen_"+self.args.dataset+".txt", "w")
			np.savetxt(a_file, class_scores)
			np.savetxt(b_file, y_pred)
			a_file.close()
			b_file.close()
